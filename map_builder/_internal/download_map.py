...
OUT_DIR = os.path.join(os.path.dirname(_HERE), "out_put")
...
def analyze_features(rgba_image: Image.Image, out_dir: str) -> None:
    """
    用 ORB+BEBLID 统计原始图像的特征点数量（仅诊断用，不再生成预处理副本）。
    ...
    from backend.vision.feat.features import create_orb_beblid_feature2d
    feature_extractor = create_orb_beblid_feature2d()
    kp_raw, _ = feature_extractor.detectAndCompute(gray, mask)
