...
def _build_manifest(mode: str, duration_ms: int) -> dict[str, Any]:
    return {
        ...
        'tool': {'runner': 'esbuild', 'script': str(BUILD_SCRIPT).replace('\\', '/')},
        'entries': [{'requestPath': request_path, 'source': str(source).replace('\\', '/'), 'output': request_path} for request_path, source in ENTRYPOINTS.items()],
    }
...
def _run_build(project_root: Path, staging_dir: Path, timeout_sec: int) -> tuple[bool, str, int]:
    ...
    command = [npm_cmd, 'run', 'build:frontend', '--', '--outdir', str(outdir)]
    start = time.perf_counter()
    try:
        completed = subprocess.run(command, cwd=str(project_root), capture_output=True, text=True, timeout=max(1, int(timeout_sec)), check=False)
...
def prebuild_frontend(project_root: Path, *, mode: Optional[str] = None) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {'mode': build_mode, 'attempted': False, 'continueStartup': True, 'activeSource': 'source', 'message': ''}
    if build_mode == 'off':
