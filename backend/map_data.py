...
_MARKERS_LITE_PATH = os.path.join(_MAP_BUILDER_DIR, 'rocom_markers_lite.json')
_MARKERS_DETAIL_PATH = os.path.join(_MAP_BUILDER_DIR, 'rocom_markers_detail.json')
_CATEGORIES_PATH = os.path.join(_MAP_BUILDER_DIR, 'categories.json')

_MAP_DATA_PATHS = (
    _MARKERS_LITE_PATH,
    _MARKERS_DETAIL_PATH,
    _CATEGORIES_PATH,
)
...
def _build_store() -> dict:
    ...
    categories = {
        str(key): {
            'name': str((value or {}).get('name') or key),
            'group': str((value or {}).get('group') or '未分组'),
        }
    ...
    return {
        ...
        'markerTypeCounts': marker_type_counts,
        'markersById': markers_by_id,
        'detailsById': details_by_id,
        'chunks': dict(chunk_index),
        'searchIndex': search_index,
        'populatedChunkKeys': populated_chunk_keys,
        'totalChunkCount': len(populated_chunk_keys),
        'totalMarkers': len(markers_by_id),
    }
...
def get_marker_manifest(force_reload: bool = False) -> dict:
    ...
    return {
        ...
        'markerTypeCounts': store['markerTypeCounts'],
        'totalMarkers': store['totalMarkers'],
        'totalChunkCount': store['totalChunkCount'],
        'populatedChunkKeys': store['populatedChunkKeys'],
    }
...
def get_marker_chunks(raw_keys) -> dict:
    ...
    return {
        ...
        'chunks': chunks,
        'totalReturned': sum(len(items) for items in chunks.values()),
    }
...
def get_marker_search_index() -> dict:
    ...
    return {
        'success': True,
        'version': store['version'],
        'items': store['searchIndex'],
        'total': len(store['searchIndex']),
    }