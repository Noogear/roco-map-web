...
WATCHDOG_HASH_MIN_MATCH_QUALITY = 0.55  # 仅当 SIFT 质量达到该阈值时才让哈希不一致参与看门狗计数
WATCHDOG_HASH_MIN_MATCH_COUNT = 6       # 仅当 SIFT 内点数达到该阈值时才让哈希不一致参与看门狗计数
WATCHDOG_MAD_THRESHOLD = 24.0           # 看门狗主MAD阈值：高于此值认定画面明显变化（死锁确诊）；18→24减少正常行走误触
WATCHDOG_INERTIAL_MIN_SIFT_FAILS = 3   # INERTIAL 坐标锁定检测：SIFT 连续失败 N 次后开始检测画面变化
