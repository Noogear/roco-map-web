...
# --- 地图文件（绝对路径，从 assets/ 加载）---
LOGIC_MAP_PATH = _os.path.join(_ROOT, "assets", "big_map.png")
DISPLAY_MAP_PATH = _os.path.join(_ROOT, "assets", "big_map-1.png")
...
COLOR_TEMP_BIAS_THRESH = 15           # R-B 通道均值差超过此值时触发护眼色温补偿（软件叠加层防蓝光）
...
RENDER_STOPPED_STILL_THRESHOLD = 10     # 玩家停止时的静止死区（吸收 SIFT 噪声，防挂机漂移）
...
SCENE_CHANGE_DEBOUNCE = 2               # 连续 N 帧检测不到小地图才触发 SCENE_CHANGE（防单帧误判）
